 ...
 */
helpers.jsonString = function (obj) {
 ...
 * @param {Number} length - The maximum length of the substring
 * @return {String} - The substring
 */
helpers.substring = function (str, length) {
...
/**
 * Format a date to Spanish format (dd-mm-yyyy hh:mm)
 *
 * @param {String} date - The date string to format
 * @return {String} - The formatted date string
 */
helpers.formatSpanishDate = function (date) {
  let fecha = new Date(date);

  let dia = fecha.getDate().toString().padStart(2, '0');
  let mes = (fecha.getMonth() + 1).toString().padStart(2, '0');
  let anio = fecha.getFullYear().toString().padStart(4, '0');
 ...
 *
 * ```handlebars
 * <!-- given that "item.name" is "foo bar baz" -->
 * {{#contains item.name "bar"}}
 *   <p>Found "bar" in {{item.name}}</p>
 * {{else}}
 *   <p>Did not find "bar" in {{item.name}}</p>
 * {{/contains}}
 * ```
 * @param {String} `str`
 * @param {String} `substr`
...
helpers.contains = function (str, substr, options) {
